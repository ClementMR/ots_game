...
minetest.register_on_mods_loaded(function()
	...
	for name, _ in pairs(recipes_cache) do
		table.insert(init_items, name)
	end
...
local function recipe_fs(fs, data)
	...
	table.insert(fs, ("label[0,3;%s]"):format(esc(data.show_usages
		and S("Usage @1 of @2", data.rnum, #data.recipes)
		or S("Recipe @1 of @2", data.rnum, #data.recipes))))

	if #data.recipes > 1 then
		table.insert(fs,
			"image_button[0,3.5;0.8,0.8;craftguide_prev_icon.png;recipe_prev;]"..
			"image_button[0.7,3.5;0.8,0.8;craftguide_next_icon.png;recipe_next;]"..
...
local function get_formspec(player)
	...
	table.insert(fs,
		...
		"no_prepend[]"..
		"bgcolor[;neither]"..
		"background[0,0;0,0;craftguide_bg.png;true]"..
		"style_type[item_image_button;padding=2]"..
		"field[3.6,1.5;2.8,1.2;filter;;"..esc(data.filter).."]"..
		"label[4.8,9.75;"..minetest.colorize("yellow", data.pagenum).." / "..
			data.pagemax.."]"..
		"image_button[6.2,1.35;0.8,0.8;craftguide_search_icon.png;search;]"..
		"image_button[6.825,1.35;0.8,0.8;craftguide_clear_icon.png;clear;]"..
		"image_button[4,9.65;0.8,0.8;craftguide_prev_icon.png;prev;]"..
		"image_button[5.75,9.65;0.8,0.8;craftguide_next_icon.png;next;]"..
	...
	if #data.items == 0 then
		table.insert(fs, "label[4,3;"..esc(S("No items to show.")).."]")
	else
		local first_item = (data.pagenum - 1) * 35
		for i = first_item, first_item + 34 do
			local item = data.items[i + 1]
			if not item then
				break
			end
			local x, y = coords(i % 35, 5)
			x = x + 3
			y = y + 2.5
...
local function on_receive_fields(player, fields)
	...
	elseif fields.recipe_next or fields.recipe_prev then
		...
		return true
	elseif fields.creative_btn then
		if not core.get_player_privs(name).creative then
			return
		end
		if data.recipes and #data.recipes > 0 then
			local stack = ItemStack(data.recipes[1].output)
			if stack and stack:get_name() ~= "" then
				local enough_space = player:get_inventory():room_for_item("main", stack)
				if enough_space then
					local max_stack = stack:get_stack_max() or 1
					player:get_inventory():add_item("main", stack:get_name().." "..max_stack)
				else
					core.chat_send_player(name, core.colorize("red", S("Not enough inventory space to add the item!")))
				end
			end
		end
		return true
...
if core.global_exists("sfinv") then
	...
	core.register_chatcommand("c_guide", {
		description = S("Open the crafting guide"),
		func = function(name)
			local player = minetest.get_player_by_name(name)
			if player then
				core.show_formspec(name, "mtg_craftguide:craftguide", get_formspec(player))
			end
		end
	})
	...
	core.register_on_player_receive_fields(function(player, formname, fields)
		...
		if fields.craftguide_btn then
			core.show_formspec(player_name, "mtg_craftguide:craftguide", get_formspec(player))
		end

		if formname == "mtg_craftguide:craftguide" then
			if on_receive_fields(player, fields) then
				core.show_formspec(player_name, "mtg_craftguide:craftguide", get_formspec(player))
			end
		end
