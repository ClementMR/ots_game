...
armor.config = {
	...
	set_multiplier = 1.1,
	water_protect = true,
	fire_protect = core.settings:get_bool("armor_fire_protect") ~= false,
	fire_protect_torch = core.settings:get_bool("armor_fire_protect_torch"),
	feather_fall = true,
	punch_damage = true,
}
...
armor.set_player_armor = function(self, player)
	...
	for i, stack in pairs(list) do
		-- Est-ce que le slot contient un élément ?
		if stack:get_count() == 1 then
			local def = stack:get_definition()
			for _, element in pairs(self.elements) do
				if def.groups["armor_"..element] then -- Est-ce que l'item est une partie d'armure ?
					if def.armor_groups then -- Est-ce que l'item possède un groupe d'armure ?
			...
			count = count + 1 -- Partie d'armure on
	...
	if use_armor_monoid then
		...
		if levels["immortal"] ~= 0 or (immortal and immortal ~= 0) then
			groups.fleshy = 0
		end
...
armor.update_skin = function(self, name)
	core.after(0, function()
		...
		if pplayer then
			self.textures[name].skin = armor.default_skin..".png"
...
armor.load_armor_inventory = function(self, player)
	...
	if inv then
		...
		if armor_list_string then
			inv:set_list("armor", self:deserialize_inventory_list(armor_list_string))
			return true
		end
	end
end
...
armor.save_armor_inventory = function(self, player)
	...
	if inv then
		local meta = player:get_meta()
		meta:set_string("3d_armor_inventory", self:serialize_inventory_list(inv:get_list("armor")))
	end
end
...
local disable_sounds = core.settings:get_bool("shields_disable_sounds", false)
armor.play_sound_effect = function(self, player, name)
