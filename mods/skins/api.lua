...
function skins.get_player_skin(player)
	...
	if meta:get("skins:skin_key") then
		-- Move player data prior July 2018 to mod storage
		storage:set_string(player_name, meta:get_string("skins:skin_key"))
		meta:set_string("skins:skin_key", "")
	end

	local skin_name = storage:get_string(player_name)
	local skin = skins.get(skin_name) or skins.get("character." .. skin_name)
