...
shooter.register_weapon = function(name, def)
	...
	core.register_tool(name.."_loaded", {
		...
		on_use = function(itemstack, user)
			...
			if inv then
				...
				if inv:contains_item("main", stack) then
					...
					core.sound_play(sound, {object=user})
...
shooter.get_configuration = function(conf)
	for k, v in pairs(conf) do
		local setting = core.settings:get("shooter_"..k)
		if type(v) == "number" then
			setting = tonumber(setting)
		elseif type(v) == "boolean" then
			setting = core.settings:get_bool("shooter_"..k)
		end
		if setting ~= nil then
			conf[k] = setting
		end
...
shooter.play_node_sound = function(node, pos)
	local item = core.registered_items[node.name]
	if item then
		if item.sounds then
...
shooter.punch_node = function(pos, spec)
	if config.enable_protection and core.is_protected(pos, spec.user) then
		return
	end
		...
		for k, v in pairs(spec.groups) do
			...
			if level >= v then
				...
				if config.node_drops then
					local object = core.add_item(pos, item)
					if object then
						object:set_velocity({
							x = math.random(-1, 1),
							y = 4,
...
shooter.punch_object = function(object, tool_caps, dir, on_blast)
	...
	if on_blast and not object:is_player() then
		...
		if ent then
			local def = core.registered_entities[ent.name] or {}
...
local function process_hit(pointed_thing, spec, dir)
	local def = core.registered_items[spec.name] or {}
...
local function fire_weapon(player, itemstack, spec, extended)
	...
	shots[spec.user] = core.get_us_time() / 1000000 + interval
	local sound = spec.sounds.shot or "shooter_pistol"
	core.sound_play(sound, {object=player})
...
shooter.fire_weapon = function(player, itemstack, spec)
	...
	if config.automatic_weapons then
		if config.admin_weapons and core.check_player_privs(name,
				{server=true}) then
			spec.automatic = true
		end
		shooting[name] = true
...
shooter.blast = function(pos, radius, fleshy, distance, user)
	...
	core.sound_play("shooter_explode", {
		pos = pos,
		gain = 10,
	...
	if config.allow_nodes and config.enable_blasting then
		if not config.enable_protection or
				not core.is_protected(pos, name) then
			core.set_node(pos, {name="shooter:boom"})
		end
	end
	...
	local objects = core.get_objects_inside_radius(pos, distance)
		...
		if shooter.is_valid_object(obj) then
			...
			if dist ~= 0 then
				...
				if shooter.is_valid_object(obj) and
						core.line_of_sight(obj_pos, blast_pos, 1) then
				...
				for x = -radius, radius do
					if (x * x) + (y * y) + (z * z) <=
						...
						if config.enable_protection then
							if not core.is_protected(vp, name) then
								data[vi] = c_air
							end
						else
							data[vi] = c_air
