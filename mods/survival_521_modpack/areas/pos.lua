...
if core.parse_relative_number then
	parse_relative_pos = function(x_str, y_str, z_str, pos)

		local x = pos and core.parse_relative_number(x_str, pos.x)
			or tonumber(x_str)
		local y = pos and core.parse_relative_number(y_str, pos.y)
			or tonumber(y_str)
		local z = pos and core.parse_relative_number(z_str, pos.z)
			or tonumber(z_str)
	...
	parse_relative_pos = function(x_str, y_str, z_str, pos)
		...
		elseif string.sub(x_str, 1, 1) == "~"
			...
			return nil, S("Relative coordinates is not supported on this server. " ..
				"Please upgrade core to 5.7.0 or newer versions.")
		end
	end
end

core.register_chatcommand("select_area", {
	params = S("<ID>"),
...
core.register_chatcommand("area_pos1", {
	params = "[X Y Z|X,Y,Z]",
	privs = {protection_bypass = true},
...
core.register_chatcommand("area_pos2", {
	params = "[X Y Z|X,Y,Z]",
	privs = {protection_bypass = true},
	description = S("Set area protection region position @1 to your"
...
core.register_chatcommand("area_pos", {
	...
	func = function(name, param)
		...
		elseif param == "get" then -- Display current area positions
			...
			if areas.pos1[name] then
				pos1str = pos1str..core.pos_to_string(areas.pos1[name])
			else
				pos1str = pos1str..S("<not set>")
			end
			if areas.pos2[name] then
				pos2str = pos2str..core.pos_to_string(areas.pos2[name])
			else
...
areas.markPos1 = function(name)
	...
	if pos ~= nil then -- Add marker
		areas.marker1[name] = core.add_entity(pos, "areas:pos1")
...
areas.markPos2 = function(name)
	...
	if pos ~= nil then -- Add marker
		areas.marker2[name] = core.add_entity(pos, "areas:pos2")
		areas.marker2[name]:get_luaentity().active = true
	end
end

core.register_entity("areas:pos1", {
	initial_properties = {
		visual = "cube",
...
core.register_entity("areas:pos2", {
	initial_properties = {
		visual = "cube",
		visual_size = {x=1.1, y=1.1},
