...
core.register_chatcommand("list_areas", {
	...
	func = function(name, param)
		local admin = core.check_player_privs(name, {protection_bypass = true})
...
core.register_chatcommand("area_info", {
	...
	func = function(name, param)
		...
		local has_high_limit = privs.protection_bypass
		local has_prot_priv = not prot_priv or privs[prot_priv]
		local can_prot = privs.protection_bypass
		...
		-- Area limit
		local area_limit_line = privs.protection_bypass and
