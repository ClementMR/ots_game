...
local function get(pos)
	...
	if pex.registered_doors[node_name] then
		...
		return {
			...
			open = function(self, player)
				...
				return door_toggle(self.pos, nil, player)
			end,
	...
	elseif pex.registered_trapdoors[node_name] then
		-- A trapdoor
		return {
			pos = pos,
			open = function(self, player)
				...
				return trapdoor_toggle(self.pos, nil, player)
			end,
			close = function(self, player)
				if not self:state() then
					return false
...
local function register_door(name, def)
	...
	core.register_lbm({
		...
		action = function(pos, node)
			...
			core.swap_node(pos, {name = name .. "_" .. new.type, param2 = p2})
			meta:set_int("state", new.state)
			-- properly place pex:hidden at the right spot
			local p3 = p2
			if new.state >= 2 then
				p3 = (p3 + 3) % 4
			end
	...
	core.register_craftitem(":" .. name, {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if core.get_item_group(core.get_node(aside).name, "door") == 1 then
				state = state + 2
				core.set_node(pos, {name = doorname .. "_b", param2 = dir})
				core.set_node(above, {name = "pex:hidden", param2 = (dir + 3) % 4})
			else
				core.set_node(pos, {name = doorname .. "_a", param2 = dir})
				core.set_node(above, {name = "pex:hidden", param2 = dir})
			end
	...
	def.mesh = "door_a.b3d"
	core.register_node(":" .. name .. "_a", table.copy(def))

	def.mesh = "door_b.b3d"
	core.register_node(":" .. name .. "_b", table.copy(def))

	def.mesh = "door_b.b3d"
	core.register_node(":" .. name .. "_c", table.copy(def))

	def.mesh = "door_a.b3d"
	core.register_node(":" .. name .. "_d", table.copy(def))

	pex.registered_doors[name .. "_a"] = true
	pex.registered_doors[name .. "_b"] = true
	pex.registered_doors[name .. "_c"] = true
	pex.registered_doors[name .. "_d"] = true
end
...
local function register_trapdoor(name, def)
	...
	pex.registered_trapdoors[name_opened] = true
	pex.registered_trapdoors[name_closed] = true
end
