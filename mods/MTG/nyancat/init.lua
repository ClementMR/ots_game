...
function nyancat.place(pos, facedir, length)
	...
	core.set_node(p, {name = "nyancat:nyancat", param2 = facedir})
	for i = 1, length do
		p.x = p.x + tailvec.x
		p.z = p.z + tailvec.z
		core.set_node(p, {name = "nyancat:nyancat_rainbow", param2 = facedir})
	end
end
...
function nyancat.generate(minp, maxp, seed)
	...
	local max_num_nyancats = math.floor(volume / (75 * 75 * 75))
...
core.register_alias("nyancat", "nyancat:nyancat")
core.register_alias("nyancat_rainbow", "nyancat:nyancat_rainbow")
