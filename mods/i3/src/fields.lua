...
IMPORT("S", "compressible", "ItemStack")
...
local function inv_fields(player, data, fields)
	...
	for field in pairs(fields) do
		if sub(field, 1, 4) == "btn_" then
			local subcat = indexof(i3.categories, sub(field, 5))
			if subcat then
				if data.subcat ~= subcat then
					data.scrbar_inv = 0
				end
		...
		elseif sub(field, 1, 8) == "setting_" then
			local setting = match(field, "_(%w+)$")
			if setting == "style" or setting == "sorting" then
				data.show_setting = setting
			end

		elseif sub(field, 1, 9) == "skin_btn_" then
			if not core.get_player_privs(name).skins then
	...
	elseif fields.buy_skins then
		if core.get_player_privs(name).skins then
			return
		end
		...
		msg(name, S("Skin access unlocked"))
...
--	No-op buttons
	if fields.player_name or fields.awards or fields.no_item or
