...
if core.global_exists("SHOOTER_ARROW_TOOL_CAPS") then
...
local function strike(arrow, pointed_thing, name)
	local puncher = core.get_player_by_name(name)
	if not puncher then
		return
	end
...
core.register_entity("shooter_crossbow:arrow_entity", {
	physical = false,
	visual = "mesh",
...
for k, color in pairs(dye_basecolors) do
	core.register_craftitem("shooter_crossbow:arrow_"..color, {
		description = S("@1 Arrow", colors[k]),
		inventory_image = get_texture("arrow_inv", color),
	})
	core.register_tool("shooter_crossbow:crossbow_loaded_"..color, {
		description = S("Crossbow"),
		inventory_image = get_texture("crossbow_loaded", color),
		groups = {not_in_creative_inventory=1},
		on_use = function(itemstack, user)
			core.sound_play("shooter_click", {object=user})
			...
			if pos and dir and yaw then
				...
				if ent then
					...
					core.sound_play("shooter_throw", {object=obj})
					local frame = get_animation_frame(dir)
...
core.register_tool("shooter_crossbow:crossbow", {
	description = S("Crossbow"),
	...
	on_use = function(itemstack, user)
		...
		if color then
			core.sound_play("shooter_reload", {object=user})
			if not core.settings:get_bool("creative_mode") then
...
core.register_alias("shooter:crossbow", "shooter_crossbow:crossbow")
for _, color in pairs(dye_basecolors) do
	core.register_alias("shooter:arrow_"..color, "shooter_crossbow:arrow_"..color)
	core.register_alias("shooter:crossbow_loaded_"..color, "shooter_crossbow:crossbow_loaded_"..color)
end
